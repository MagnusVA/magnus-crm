...
function buildReportWorksheet(
	...
	addTableSection(worksheet, merges, cursor, {
		...
		emptyText: "No lead generator activity in this scope.",
		numberFormats: {
			0: "0",
			3: "#,##0",
			4: "#,##0",
	...
	const workerPerformanceRange = addTableSection(worksheet, merges, cursor, {
		...
		emptyText: "No worker activity in this scope.",
		numberFormats: {
			4: "#,##0",
			5: "#,##0",
			6: "#,##0",
...
function addTitleBlock(
	...
	const metadataRows = [
		...
		`Scope: ${sheet.scopeKind === "worker" ? "Worker" : "Team"} = ${
			sheet.scopeLabel
		}`,
...
function addSummaryKpiSection(
	...
	const cards = [
		{ label: "SUBMISSIONS", value: summary.submissions, format: "#,##0" },
		{
			...
			value: summary.uniqueProspects,
			format: "#,##0",
		},
		{ label: "DUPLICATES", value: summary.duplicates, format: "#,##0" },
		{
